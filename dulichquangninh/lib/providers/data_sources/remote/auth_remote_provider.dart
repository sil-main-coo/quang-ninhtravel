...
class AuthRemoteProvider {
  final refUsersDB = FirebaseDatabase.instance.ref().child('users');
  ...
  Future<User> signIn(User account) async {
    ...
    try {
      ...
      if (authUserName.value != null) {
        final mapUser = Map<String, dynamic>.from(authUserName.value as Map);
  ...
  Future<DataSnapshot> getUserByPhone(String phone) async {
    try {
      final result =  await refUsersDB.orderByChild('phone').equalTo(phone).once();
